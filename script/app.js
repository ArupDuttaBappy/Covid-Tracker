...
let sortable_country_matrix = new Array(194).fill(0).map(() => new Array(7).fill(0));

async function fetch_api_country_summary_data(){
  let requestOptions = {
    method: 'GET',
    redirect: 'follow'
  };
  let local = await fetch("https://api.covid19api.com/summary", requestOptions)
  ...
  return sortable_country_matrix;
}

async function start() {
  country_list_data = await fetch_api_country_summary_data();
}
start();
